...
import {
  ...
  Paper, Typography, Box, Button, ButtonGroup, CircularProgress, Stack
} from '@mui/material';
...
const fetchEntries = async () => {
  ...
  try {
    const res = await axios.get(`http://localhost:3001/api/tasksheetEntries/user/${userId}`);
    ...
    console.log(
      'Raw entries:',
      res.data.map(e => `${e.id}: ${e.entry_date}`)
    );
    console.log(
      'Sorted by created_at:',
      sortedEntries.map(e => `${e.id}: ${e.created_at.format('YYYY-MM-DD HH:mm:ss')}`)
    );
...
const getCategoryName = (id) => {
  ...
  console.log('Sample category:', taskCategories[0]);
}, [entries, projects, taskCategories]);
...
const getFilteredEntries = () => {
  ...
  switch (filterRange) {
    case 'TODAY':
      filtered = entries.filter(e => dayjs(e.entry_date).isSame(now, 'day'));
      break;
    case 'WEEK':
      filtered = entries.filter(e => dayjs(e.entry_date).isAfter(now.subtract(7, 'day')));
      break;
    case 'MONTH':
      filtered = entries.filter(e => dayjs(e.entry_date).isAfter(now.subtract(1, 'month')));
      break;
    case '3MONTH':
      filtered = entries.filter(e => dayjs(e.entry_date).isAfter(now.subtract(3, 'month')));
      break;
    case '6MONTH':
      filtered = entries.filter(e => dayjs(e.entry_date).isAfter(now.subtract(6, 'month')));
      break;
    default:
      filtered = [...entries];
      break;
  ...
  return filtered.sort((a, b) => dayjs(b.entry_date).valueOf() - dayjs(a.entry_date).valueOf());
};

  return (<> 
   <Box sx={{ p: 3 }}>
      <Typography variant="h6" gutterBottom>Tasksheet Entries</Typography>
